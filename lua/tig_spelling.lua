...
local function filter(input, env)
	...
	if spelling_states then
				...
				elseif script_text:find("^[a-z]*") and not script_text:find("%p$") and env.engine.context:get_option("rvl_zhuyin") then
					...
					if script_text:find("^[a-z]*") and not script_text:find("%p$") or script_text:find("^([%/])[a-z]*") and not script_text:find("%p$") then
						...
						if add_comment~=nil or add_comment~="" then
							if cand.comment == "" then
								yield(Candidate(spelling_kw, cand.start, cand._end, cand.text,add_comment))
							else
								-- 编码栏同时显示组合编码
					...
					-- 	yield(cand)
						...
						if cand.comment:find("^[%~][a-zA-Z0-9]") then
							add_comment = get_tricomment(cand, env)
							-- yield(Candidate(spelling_kw, cand.start, cand._end, cand.text,add_comment:gsub(border_end,"  ") .. cand.comment .. " " .. border_end))
							yield(Candidate(spelling_kw, cand.start, cand._end, cand.text,add_comment:gsub(border_end,"").. border_end))
						else
							yield(cand)
						end
					end
		...
		elseif script_text:find("^([%~])[a-z]*") and not script_text:find("%p$") and env.engine.context:get_option("rvl_zhuyin") then
		-- if script_text:find("^([%~])[a-z]*") and not script_text:find("%p$") and env.engine.context:get_option("rvl_zhuyin") then
			for cand in input:iter() do
				if isgb2312(cand,env)==1 and env.engine.context:get_option("GB2312") or not env.engine.context:get_option("GB2312") then
					table.insert(CandidateText,cand.text)
					local code_comment=env.code_rvdb:lookup(cand.text)
